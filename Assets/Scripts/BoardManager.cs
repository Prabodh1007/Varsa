...
public class BoardManager : MonoBehaviour
{
    ...
    /// </summary>
    public Dictionary<Tile, List<PlayerStone>> tileOccupants = new Dictionary<Tile, List<PlayerStone>>();

    public void RegisterPieceOnTile(Tile t, PlayerStone stone)
    {
        if (t == null) return;
        if (!tileOccupants.ContainsKey(t))
            tileOccupants[t] = new List<PlayerStone>();
        if (!tileOccupants[t].Contains(stone))
            tileOccupants[t].Add(stone);
    }

    public void UnregisterPieceFromTile(Tile t, PlayerStone stone)
    {
        if (t == null) return;
        if (tileOccupants.ContainsKey(t))
        {
            tileOccupants[t].Remove(stone);
            if (tileOccupants[t].Count == 0)
                tileOccupants.Remove(t);
        }
    }
